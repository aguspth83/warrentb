...
# -----------SQL-----------
resultados_total = sqlite3.connect("resultados1mbb.db")
...
async def run_bot():
    ...
    while True:
        ...
        df['signal'] = 0.0
        df['signal'] = np.where(((df['MACD'] > df['Signal_line']) & (df['close'] < df['Lower_Band'])), 1.0, 0.0)
        df['signal'] = np.where(((df['MACD'] < df['Signal_line']) & (df['close'] > df['Upper_Band'])), -1.0,
                                df['signal'])
