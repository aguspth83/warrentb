    ...
    while True:
        ...
        for i, row in df.tail(1).iterrows():
            ...
            if row['signal'] == 1 and buy_signal == False:
                cond1_buy = f"✅ Buy condition 1 at {last_price} (MACD {(df['MACD'][-1] - df['Signal_line'][-1]).round(2)})"
                await send_telegram_message(cond1_buy)
                print(cond1_buy)
                n_future_candles = 7  # Número de velas futuras a verificar
            ...
            # Check for sell signal and trailing stop loss
                ...
                cond1_sell = f"✅ Sell condition 1 at {last_price} (highest price: {highest_price} - buy price: {buy_price} - trailing stoploss: {t_stoploss_percent})"
